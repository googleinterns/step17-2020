// // Copyright 2019 Google LLC
// //
// // Licensed under the Apache License, Version 2.0 (the "License");
// // you may not use this file except in compliance with the License.
// // You may obtain a copy of the License at
// //
// //     https://www.apache.org/licenses/LICENSE-2.0
// //
// // Unless required by applicable law or agreed to in writing, software
// // distributed under the License is distributed on an "AS IS" BASIS,
// // WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// // See the License for the specific language governing permissions and
// // limitations under the License.

package com.google.sps.servlets;

import com.google.appengine.api.datastore.DatastoreService;
import com.google.appengine.api.datastore.DatastoreServiceFactory;
import com.google.appengine.api.datastore.Entity;
import com.google.appengine.api.datastore.PreparedQuery;
import com.google.appengine.api.datastore.Query;
import com.google.gson.Gson;
import com.google.sps.data.*;
import java.io.IOException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

// This servlet will be used for retrieving the user's username given their email
@WebServlet("/get-username")
public class GetUsername extends HttpServlet {

  @Override
  public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
    response.setContentType("application/json");
    // from datastore, retrieve username
    Gson gson = new Gson();
    // look for specific entity with "email" for this user's email, and get their username
    String email = getParameter(request, "email", "");
    Query query = new Query("User").addFilter("email", Query.FilterOperator.EQUAL, email);
    PreparedQuery results = getDatastore().prepare(query);
    for (Entity entity : results.asIterable()) {
      System.out.println(entity);
      if (entity.getProperty("email").equals(email)) {
        // get the username linked
        String username = entity.getProperty("username").toString();
        String json = gson.toJson(username);
        // send username to js
        response.getWriter().println(json);
        break;
      }
    }
  }

  private static DatastoreService getDatastore() {
    return DatastoreServiceFactory.getDatastoreService();
  }

  private String getParameter(HttpServletRequest request, String name, String defaultValue) {
    String value = request.getParameter(name);
    return (value == null) ? defaultValue : value;
  }
}
